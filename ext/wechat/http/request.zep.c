...
PHP_METHOD(Wechat_Http_Request, xmlToArray) {
	...
	if (!(ZEPHIR_IS_EMPTY(xmlStr))) {
		ZEPHIR_CALL_FUNCTION(NULL, "libxml_disable_entity_loader", NULL, 37, ZEPHIR_GLOBAL(global_true));
		zephir_check_call_status();
		ZEPHIR_SINIT_VAR(_0$$3);
		ZVAL_STRING(&_0$$3, "SimpleXMLElement", 0);
		ZEPHIR_SINIT_VAR(_1$$3);
		ZVAL_LONG(&_1$$3, 16384);
		ZEPHIR_CALL_FUNCTION(&xmlData, "simplexml_load_string", NULL, 38, xmlStr, &_0$$3, &_1$$3);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&requestInfo, "get_object_vars", NULL, 39, xmlData);
		zephir_check_call_status();
	} else {
