...
ZEPHIR_INIT_CLASS(Wechat_Core_Http) {
	...
	ZEPHIR_REGISTER_CLASS(Wechat\\Core, Http, wechat, core_http, wechat_core_http_method_entry, 0);

	/**
	 * 超时时间
	 */
	zend_declare_property_long(wechat_core_http_ce, SL("_timeout"), 10, ZEND_ACC_PRIVATE TSRMLS_CC);
...
PHP_METHOD(Wechat_Core_Http, request) {
	...
	if (ZEPHIR_IS_EMPTY(type)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_exception_get_default(TSRMLS_C), "type must be required ", "wechat/core/http.zep", 46);
		return;
	}
	if (ZEPHIR_IS_EMPTY(url)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_exception_get_default(TSRMLS_C), "url must be required", "wechat/core/http.zep", 51);
		return;
	}
	...
	if (_0) {
		...
		if (_3$$5) {
			ZEPHIR_CALL_FUNCTION(&paramUrl, "http_build_query", NULL, 30, params);
	...
	ZEPHIR_CALL_FUNCTION(&curlHandle, "curl_init", NULL, 31);
	zephir_check_call_status();
	ZEPHIR_SINIT_VAR(_15);
	ZVAL_LONG(&_15, 10002);
	ZEPHIR_CALL_FUNCTION(NULL, "curl_setopt", &_16, 32, curlHandle, &_15, url);
	zephir_check_call_status();
	ZEPHIR_SINIT_NVAR(_15);
	ZVAL_LONG(&_15, 19913);
	ZEPHIR_SINIT_VAR(_17);
	ZVAL_LONG(&_17, 1);
	ZEPHIR_CALL_FUNCTION(NULL, "curl_setopt", &_16, 32, curlHandle, &_15, &_17);
	zephir_check_call_status();
	ZEPHIR_SINIT_NVAR(_15);
	ZVAL_LONG(&_15, 64);
	ZEPHIR_CALL_FUNCTION(NULL, "curl_setopt", &_16, 32, curlHandle, &_15, ZEPHIR_GLOBAL(global_false));
	zephir_check_call_status();
	ZEPHIR_SINIT_NVAR(_15);
	ZVAL_LONG(&_15, 81);
	ZEPHIR_CALL_FUNCTION(NULL, "curl_setopt", &_16, 32, curlHandle, &_15, ZEPHIR_GLOBAL(global_false));
	...
	ZEPHIR_SINIT_NVAR(_15);
	ZVAL_LONG(&_15, 13);
	ZEPHIR_CALL_FUNCTION(NULL, "curl_setopt", &_16, 32, curlHandle, &_15, _18);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(_20);
	...
	if (ZEPHIR_IS_STRING(_20, "post")) {
		...
		ZVAL_LONG(&_22$$11, 1);
		ZEPHIR_CALL_FUNCTION(NULL, "curl_setopt", &_16, 32, curlHandle, &_21$$11, &_22$$11);
		zephir_check_call_status();
		ZEPHIR_SINIT_NVAR(_21$$11);
		ZVAL_LONG(&_21$$11, 10015);
		ZEPHIR_CALL_FUNCTION(NULL, "curl_setopt", &_16, 32, curlHandle, &_21$$11, postData);
		zephir_check_call_status();
	}
	ZEPHIR_CALL_FUNCTION(&content, "curl_exec", NULL, 33, curlHandle);
	zephir_check_call_status();
	if (ZEPHIR_IS_FALSE_IDENTICAL(content)) {
		ZEPHIR_CALL_FUNCTION(&_23$$12, "curl_error", NULL, 34, curlHandle);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(_24$$12);
		ZEPHIR_CONCAT_SV(_24$$12, "Curl error: ", _23$$12);
		zend_print_zval(_24$$12, 0);
		ZEPHIR_CALL_FUNCTION(NULL, "curl_close", &_25, 35, curlHandle);
		zephir_check_call_status();
		RETURN_MM_BOOL(0);
	}
	ZEPHIR_CALL_FUNCTION(NULL, "curl_close", &_25, 35, curlHandle);
